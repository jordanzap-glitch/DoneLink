...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['edit']) && !isset($_POST['delete'])) {
    ...
    $stmtCustomer = $conn->prepare("INSERT INTO tbl_customer (firstname, middlename, lastname, address, subscription_id, email, username, password, datecreated) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    
    // Bind parameters for tbl_customer
    $stmtCustomer->bind_param("ssssissss", $firstName, $middleName, $lastName, $address, $subscriptionId, $email, $username, $password, $dateCreated);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit'])) {
    ...
    if (!empty($password)) {
        ...
        $stmtEdit = $conn->prepare("UPDATE tbl_customer SET firstname=?, middlename=?, lastname=?, address=?, subscription_id=?, email=?, username=?, password=? WHERE id=?");
        $stmtEdit->bind_param("ssssissssi", $firstName, $middleName, $lastName, $address, $subscriptionId, $email, $username, $password, $id);
    } else {
        // If no new password is provided, update without changing the password
        $stmtEdit = $conn->prepare("UPDATE tbl_customer SET firstname=?, middlename=?, lastname=?, address=?, subscription_id=?, email=?, username=? WHERE id=?");
        $stmtEdit->bind_param("ssssisssi", $firstName, $middleName, $lastName, $address, $subscriptionId, $email, $username, $id);
    }
...
$result = $conn->query("SELECT id, firstname, middlename, lastname, address, subscription_id, email, username, datecreated FROM tbl_customer");
...
<body>
    ...
    <script>
        ...
        function openEditModal(id, firstName, middleName, lastName, address, subscriptionId, email, username) {
            $('#editId').val(id);
